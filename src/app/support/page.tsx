					...
					<div className="container">
						...
						<div className="max-w-[800px] mx-auto space-y-4 px-4 sm:px-0">
							{faqs.map((faq, index) => (
								<div
									key={index}
									className={`bg-background border border-border rounded-xl overflow-hidden transition-colors duration-300 hover:border-[var(--color-blue)] ${openIndex === index ? "border-[var(--color-secondary)]" : ""}`}
								>
									<button
										onClick={() => toggleFaq(index)}
										className="w-full flex justify-between items-center gap-4 p-6 cursor-pointer text-left"
									>
										<h3 className="text-lg font-semibold text-text m-0">{faq.question}</h3>
									...
									>
										<p className="text-text-secondary leading-relaxed px-6 pb-6">{faq.answer}</p>
									</div>
								</div>
							))}
							</div>
						...
						<form onSubmit={handleSubmit} className="max-w-[600px] mx-auto space-y-6 px-4 sm:px-0">
							<div>
								...
								<input
									...
									name="name"
									required
									className="w-full px-4 py-3 bg-surface border border-border rounded-lg text-text text-base transition-all duration-300 focus:outline-none focus:border-[var(--color-blue)] focus:shadow-[0_0_0_3px_rgba(59,130,246,0.1)]"
								/>
							</div>
							<div>
								<label htmlFor="email" className="block mb-2 text-text font-medium">Email</label>
								<input
									type="email"
									id="email"
									name="email"
							...
							<div>
								<label htmlFor="subject" className="block mb-2 text-text font-medium">Subject</label>
								<select
									id="subject"
									name="subject"
									required
									className="w-full px-4 py-3 bg-surface border border-border rounded-lg text-text text-base transition-all duration-300 focus:outline-none focus:border-[var(--color-blue)] focus:shadow-[0_0_0_3px_rgba(59,130,246,0.1)] cursor-pointer"
								>
							...
							<div>
								<label htmlFor="message" className="block mb-2 text-text font-medium">Message</label>
								<textarea
									id="message"
									name="message"
									rows={5}
									required
									className="w-full px-4 py-3 bg-surface border border-border rounded-lg text-text text-base transition-all duration-300 focus:outline-none focus:border-[var(--color-blue)] focus:shadow-[0_0_0_3px_rgba(59,130,246,0.1)] resize-y min-h-[120px]"
								></textarea>
							</div>
							<button
								type="submit"
